...
class Qwen2MLP(nn.Module):
    def __init__(self, config):
        ...
        self.gate_proj = nn.Linear(self.hidden_size, self.intermediate_size, bias=False) # out: [batch, seq_len, intermediate_size]
        self.up_proj = nn.Linear(self.hidden_size, self.intermediate_size, bias=False) # out: [batch, seq_len, intermediate_size]
        self.down_proj = nn.Linear(self.intermediate_size, self.hidden_size, bias=False) # out: [batch, seq_len, hidden_size]
        self.act_fn = ACT2FN[config.hidden_act]

    def forward(self, x):
        down_proj = self.down_proj(self.act_fn(self.gate_proj(x)) * self.up_proj(x)) # out: [batch, seq_len, hidden_size], keep same as input
        return down_proj
